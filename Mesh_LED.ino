...
void blinkLed(int times, int delayTime); // Prototipo para parpadear el LED
...
void blinkLed(int times, int delayTime) {
  for (int i = 0; i < times; i++) {
    ledcWrite(RED_CHANNEL, 255 * 8191 / 255); // Enciende el LED rojo
    delay(delayTime);
    ledcWrite(RED_CHANNEL, 0); // Apaga el LED
    delay(delayTime);
  }
}
...
void receivedCallback(uint32_t from, String &msg) {
  ...
  // Controlar el estado del LED en función del estado de monitoreo
  if (status == "Apagado") {
    ledEnabled = false; // Deshabilitar el LED
    blinkLed(3, 500); // Parpadear el LED 3 veces antes de apagar
  } else {
    ledEnabled = true; // Habilitar el LED
  }
...
void setup() {
  ...
  WiFi.begin("CASA-6", "HG.27.SOCORRO"); // Conectar a WiFi para obtener NTP
